...
(function (global) {
     ...
     * @type {Object}
     */
    var modModules = window.EHU_MOD_MODULES = {};

    // [EHU] 维护一个被依赖module-其宿主module的映射
    var depModules = window.EHU_DEP_MODULES = {};
    function addDepModule(dep, module) {
        !depModules[dep] && (depModules[dep] = []);
        depModules[dep].indexOf(module) === -1 && depModules[dep].push(module);
    }

    // [EHU] 维护一个url-moduleId的映射
    window.EHU_URL_MODULE_ID_MAP = {};
    function addURLModuleIdMap(src, mod) {
        var url = src.replace(/^.*?\/?((src|dep)\/[^?]*).*$/, '$1');
        var arr = mod.split('!');
        switch (arr[1] && arr[0]) {
            case 'fc-component-ria/component':
                url += '.component.html';
        }
        window.EHU_URL_MODULE_ID_MAP[url] = mod;
        return url;
    }
        ...
        each(deps, function (depId, index) {
            ...
            if (absId && !BUILDIN_MODULE[absId]) {
                ...
                if (idInfo.res) {
                    ...
                    addDepModule(idInfo.mod + '!' + normalize(idInfo.res, id), id);
                }
                else {
                    addDepModule(absId, id);
                }
     ...
     * @type {Object}
     */
    var loadingModules = window.EHU_LOADING_MODULES = {};
    ...
    function loadResource(pluginAndResource, baseId) {
        ...
        function load(plugin) {
            ...
            // [EHU] 保存下来已经load的文件的url=>moduleId映射
            addURLModuleIdMap(pluginRequire.toUrl(idInfo.res), idInfo.mod + '!' + idInfo.res);
    ...
    function createLocalRequire(baseId) {
        !window.EHU_REQUIRED_CACHE && (window.EHU_REQUIRED_CACHE = []);
        EHU_REQUIRED_CACHE.push({});
        var requiredCache = EHU_REQUIRED_CACHE[EHU_REQUIRED_CACHE.length-1];
    ...
    function createScript(moduleId, onload) {
        ...
        // [EHU] 保存下来已经load的文件的url=>moduleId映射
        addURLModuleIdMap(script.src, moduleId);
